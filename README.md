...
- **Owner controls (trip dashboard sidebar):**
  ...
  - **Guests** — trip team (owner + collaborators), add or remove guest names for the public list, invite collaborators by email, and see voting participation when available. Guests cannot add themselves on the public page; if a guest is missing, the trip owner or a collaborator must add them from this panel (names are unique per trip).
